...
- [Assets/Editor/SpriteImportConfigurator.cs](Assets/Editor/SpriteImportConfigurator.cs) — menu **Retroself → Configure Character Sprites**. Aplica `TextureImporter` settings de pixel art (PPU per-folder, FilterMode Point, no compression). O slicing Multiple-mode também é aplicado por compatibilidade, mas o `SimpleSpriteAnimator` em runtime usa só `Texture2D` raw + `Sprite.Create()` — não depende dos slices. Idempotente.
- [Assets/Editor/AnimatorBuilder.cs](Assets/Editor/AnimatorBuilder.cs) — menu **Retroself → Build Character Animators**. **DESATIVADO** no fluxo atual: `Memory01Builder` agora usa `SimpleSpriteAnimator` (runtime) em vez de AnimatorController. O builder existe ainda mas não é chamado — pode ser removido junto com `Assets/Animations/`, `PlayerAnimDriver` e `BullyAnimDriver` quando for fazer cleanup.

## Working style

- **Authorship convention:** sprites and music are autoral (made by the user) and added in post. We work with placeholders or empty `SpriteRenderer`/`AudioSource` slots — never generate or commit asset binaries. The `Audio/` and `Sprites/` folders are intentionally empty. Quando for trocar placeholders pelos sprites finais, siga [docs/sprites-import.md](docs/sprites-import.md) — guia com import settings (PPU 16, Point, no compression), tabela de placeholders e workflow de Tile Palette.
...
**Pixel art autoral + SimpleSpriteAnimator (Sprint 3.2):** os 3 personagens visíveis na fase usam sprites autorais do Craftpix, fatiados em runtime via `Sprite.Create()` e ciclados pelo componente `SimpleSpriteAnimator`:
- `PlayerYoung/Body` → `Assets/Sprites/criancas/Child_3/` (Idle 6 frames, Walk 10 frames; sem Jump.png — fallback p/ Idle). PPU 64.
- `PlayerAdult/Body` → `Assets/Sprites/mendigos/Homeless_1/` (Idle 6, Walk 10, Jump 1). PPU 28.
- `Bully/Body` → `Assets/Sprites/gangsters/Gangsters_2/` (porteiro reframed: é o monstro do tutorial, alinha com a fala "o porteiro não me deixou entrar"). Idle 7, Walk 10, Jump 1. PPU 32.

Pipeline simples: `Retroself → Build Memory_01_Patio Tutorial` chama `SpriteImportConfigurator` (PPU/Point/no-compression) e instancia `SimpleSpriteAnimator` com referências às `Texture2D` raw + frame counts. **Não usa AnimatorController nem `.anim` files** — fatia em runtime na primeira execução, escolhe Idle/Walk/Jump por estado do `PlayerController`/`Rigidbody2D`. O pipeline anterior (`AnimatorBuilder` + `.controller` + sub-sprites Multiple-mode) ficava deixando o `SpriteRenderer` mostrar a sheet inteira como um sprite gigante (vários "Woodies" lado-a-lado na cena); `Sprite.Create` direto da textura raw evita o problema. Body GameObject usa pivot Bottom + `localPosition.y = -collider.height/2` pra alinhar pés ao collider.
